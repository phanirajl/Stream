		...
		case msg, ok := <-consumer.Messages():
			...
			if (i % 30) == 0 {
				...
				if err != nil {
					...
					// Todo: P1 There is no cleanup of the files happening -- Top Priority
				}
			...
			if ok {
				...
				if err != nil {
					...
					// TODO: This consumer mark offset has to be removed -- it was put in because of bad records -- also make it exit and not continue
				...
				if err != nil {
					if len(string(sdc)) > 200 {
						fmt.Println(fmt.Sprintf("JSON Decode error %v -- Payload : %v ", err, string(sdc)[0:200]))
					} else {
						fmt.Println(fmt.Sprintf("JSON Decode error %v -- Payload : %v ", err, string(sdc)))
					}
					// TODO: This consumer mark offset has to be removed -- it was put in because of bad records -- also make it exit and not continue
					consumer.MarkOffset(msg, "")
					continue
				...
				fmt.Println("Problem with message that came in -- Channel says ok is false")
			}
